...
import { WEBUI_API_BASE_URL, WEBUI_BASE_URL, USE_LOCAL_DATA } from '../constants';
...
export const generateChatCompletion = async (params, callback, signal) => {
  let providerName = localStorage.getItem('llm_provider') || 'demo';

  // If we are in demo/local data mode, override any non-custom provider to 'demo'
  if (isDemoMode() && providerName !== 'custom') {
    providerName = 'demo';
